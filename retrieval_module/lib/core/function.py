...
def valid_model(dataLoader, model, criterion_img, criterion_txt, cfg, device):
    ...
    with torch.no_grad():
        for i, (_, _, image, caption) in enumerate(dataLoader):
            ...
            logits_per_image *= (np.exp(0.01) / np.exp(cfg.TRAIN.TEMPERATURE))
            logits_per_text *= (np.exp(0.01) / np.exp(cfg.TRAIN.TEMPERATURE))
